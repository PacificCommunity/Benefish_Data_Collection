...
##
   ...
   ##
      ...
      names(All_Data) <- str_replace_all(Contents$Dframe, "Clean_","")
      
      Countries_and_Tables <- do.call(rbind, lapply(names(All_Data), function(First_Level){
                                                      Dlist <- All_Data[[First_Level]]
                                                      ##
                                                      ##    HERE
                                                      ##
                                                      X <- data.frame(Captured_Tables = names(Dlist))
                                                      X$PICT <- str_replace_all(First_Level, "Clean_","")
                                                      return(X)
                                                   })
                                      )
      
      
Peek <- All_Data[[First_Level]]      
