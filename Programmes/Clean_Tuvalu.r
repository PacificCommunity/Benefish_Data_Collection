...
##
   ...
   ##
      X <- Tuvalu[["Estimates by the Benefish studies of annual fisheries harvestsXXTable14-5"]]
      for(i in 2:nrow(X))
      {
   ...
   ##
      X <- Tuvalu[["Fishing contribution to GDP in 2021 using an alternative approachXXTable14-7"]]
      names(X) <- X[1,]
   ...
   ##
   ##    Catches of the major fisheries in Tuvalu - Table6-1 & Table6-2
   ##
      ##
      ##    Table 6-2
      ##
         X <- Tuvalu[["Catch of the foreign longliners in the PNG EEZXXTable14-3"]]
         X$V1[1] <- "Year"
         names(X) <- X[1,]
      ...
      ##    Table 6-1
      ##
         Y <- Tuvalu[["Catch of the foreign purse seiners in the PNG EEZ XXTable14-2"]]
         Y$V1[1] <- "Year"
         names(Y) <- Y[1,]
      ...
      ##    Locals
      ##
         Z <- Tuvalu[["Catches of the locally based purse seine fleet (t)XXTable14-1"]]
         Z$V1[1] <- "Species"
         names(Z) <- Z[1,]
         ...
         Clean_Tuvalu[["Catches by Method"]] <- Combined_Table[,c("Measure","Table", "Method", "Species", "Year", "Unit", "Value")]
         Clean_Tuvalu[["Catches by Method"]] <- aggregate(Value ~ Measure + Table + Method + Species + Year + Unit,
                                                                     data = Clean_Tuvalu[["Catches by Method"]],
                                                                     FUN  = sum,
                                                                     na.action = NULL)


   ##
   ##    Fishing contribution to Tuvalu GDP in 2021 - Table7-7
   ##
      X <- Tuvalu[["Fishing contribution to PNG\x92s GDP XXTable14-6"]]
      X$V1[1] <- "Harvest sector"
      names(X) <- X[1,]
      ...
      X$Unit    <- "K"
      Clean_Tuvalu[["Fishing contribution to GDP"]] <- X[,c("Measure","Table", "Harvest_Sector", "Year", "Unit", "Value")]
