...
##
   ...
   ##
      ...
      ##    Table 6-2
      ##
         X <- Tokelau[["Catch of the foreign longliners in the PNG EEZXXTable14-3"]]
         X$V1[1] <- "Year"
         names(X) <- X[1,]
      ...
      ##    Table 6-1
      ##
         Y <- Tokelau[["Catch of the foreign purse seiners in the PNG EEZ XXTable14-2"]]
         Y$V1[1] <- "Year"
         names(Y) <- Y[1,]
      ...
      ##    Locals
      ##
         Z <- Tokelau[["Catches of the locally based purse seine fleet (t)XXTable14-1"]]
         Z$V1[1] <- "Species"
         names(Z) <- Z[1,]
         ...
         Clean_Tokelau[["Catches by Method"]] <- aggregate(Value ~ Measure + Table + Method + Species + Year + Unit,
                                                                     data = Clean_Tokelau[["Catches by Method"]],
                                                                     FUN  = sum,
                                                                     na.action = NULL)
   ...
   ##
      ...
      X$Measure <- "Fishing contribution to GDP - VAR Method"
      X$Unit  = ifelse(X$variable == "VAR", "Proportion", "NZ$")
      Clean_Tokelau[["Fishing contribution to GDP - VAR Method"]] <- X[,c("Measure","Table", "Harvest_Sector", "Year", "Unit", "Value")]
